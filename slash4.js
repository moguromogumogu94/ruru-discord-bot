...
function recordMessage(m,guild,name){
  ...
  return { id, channel_id:channelId, channel:name||channelId,
    author:m.author?.globalName||m.author?.global_name||m.author?.username||'不明',
    author_id:m.author?.id, bot:!!m.author?.bot,
    date:m.timestamp||m.createdAt?.toISOString(), edited_at:m.edited_timestamp||m.editedAt?.toISOString()||null,
    content:clean([m.content,embeds].filter(Boolean).join('\n'),1800),
...
async function recentMessages(channel,beforeId){
  try{const msgs=await channel.messages.fetch({limit:10,...(beforeId?{before:beforeId}:{})});
    return [...msgs.values()].reverse().filter(m=>!m.author.bot).map(m=>recordMessage(m,channel.guild,m.channel.name)).filter(x=>x.content).slice(-8).map(x=>({...x,content:clean(x.content,700)}));
  }catch{return [];}
}
async function searchServer(plan,channel,member,omitId){
  ...
  const ids=requested.length?requested.filter(id=>allowed.has(id)):[...allowed.keys()].slice(0,90);
  const queries=[...new Set((Array.isArray(plan.queries)?plan.queries:[plan.query||'']).map(q=>clean(q,100).trim()))].slice(0,2);
  ...
  for(const q of queries){
    ...
    if(q) params.set('content',q); for(const id of ids) params.append('channel_id',id);
    try{const data=await client.rest.get(`/guilds/${channel.guild.id}/messages/search`,{query:params});
      for(const m of (Array.isArray(data.messages)?data.messages.flat():[]).slice(0,30)){
        if(!m?.id||m.id===omitId||m.author?.id===client.user.id||!allowed.has(m.channel_id)) continue;
        const item=recordMessage(m,channel.guild,allowed.get(m.channel_id)?.name); if(item.content) found.set(item.id,item);
      }
    }catch(e){errors.push(safeErr(e));}
  }
  return {type:'server',status:errors.length?(found.size?'partial':'error'):'ok',queries,
    records:[...found.values()].sort((a,b)=>String(b.date).localeCompare(String(a.date))).slice(0,14),errors,
...
async function searchNews(query){
  const u=new URL('https://news.google.com/rss/search');u.search=new URLSearchParams({q:clean(query,120),hl:'ja',gl:'JP',ceid:'JP:ja'}).toString();
  const r=await fetch(u,{signal:AbortSignal.timeout(10000)});if(!r.ok)throw new Error(`News ${r.status}`);const xml=await r.text();
  const records=[...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)].slice(0,8).map(m=>({title:clean(parseTag(m[1],'title'),300),url:parseTag(m[1],'link'),date:parseTag(m[1],'pubDate'),source:parseTag(m[1],'source')})).filter(x=>x.title&&/^https:\/\//.test(x.url));
  return {type:'news',status:'ok',checked_at:new Date().toISOString(),records,note:'ニュース見出し・配信元・日時・リンクです。記事本文は未確認です。'};
}

function externalSafe(question){return !/discord\.com\/channels|<@|\b\d{16,20}\b|[\w.+-]+@[\w.-]+\.[a-z]{2,}|このサーバー|Discord内|ディスコード内|この部屋|別のチャンネル|メンバーの発言|過去の投稿/i.test(question);}
async function searchWeb(query,question){
  query=clean(query,160).trim(); if(!query||!externalSafe(question)) return {type:'web',status:'blocked',records:[],note:'内部情報・個人情報を外部検索へ送らないため検索を実行していません。'};
  const r=await fetch(`https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`,{headers:{'user-agent':'Mozilla/5.0'},signal:AbortSignal.timeout(12000)});
  if(!r.ok) throw new Error(`Web ${r.status}`); const html=await r.text(); const records=[];
  for(const m of html.matchAll(/<a[^>]+class="[^"]*result__a[^"]*"[^>]+href="([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi)){
    let href=decodeHtml(m[1]); try{const u=new URL(href,'https://duckduckgo.com');const uddg=u.searchParams.get('uddg');if(uddg)href=decodeURIComponent(uddg);}catch{}
    if(!/^https?:\/\//i.test(href))continue; records.push({title:clean(stripHtml(m[2]),300),url:clean(href,1000)}); if(records.length>=8)break;
  }
...
async function searchWeather(question){
  ...
  const gr=await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(place)}&count=1&language=ja&format=json`,{signal:AbortSignal.timeout(10000)});
  if(!gr.ok)throw new Error(`Geo ${gr.status}`);const gj=await gr.json();const loc=gj.results?.[0];
  if(!loc)return {type:'weather',status:'empty',place,records:[],note:'場所を特定できませんでした。'};
  const params=new URLSearchParams({latitude:String(loc.latitude),longitude:String(loc.longitude),timezone:'Asia/Tokyo',forecast_days:'7',daily:'weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max,precipitation_sum,wind_speed_10m_max'});
  const wr=await fetch(`https://api.open-meteo.com/v1/forecast?${params}`,{signal:AbortSignal.timeout(10000)});if(!wr.ok)throw new Error(`Weather ${wr.status}`);const w=await wr.json();
  const rows=(w.daily?.time||[]).map((d,i)=>({date:d,weather_code:w.daily.weather_code?.[i],temp_max:w.daily.temperature_2m_max?.[i],temp_min:w.daily.temperature_2m_min?.[i],precip_probability_max:w.daily.precipitation_probability_max?.[i],precipitation_sum:w.daily.precipitation_sum?.[i],wind_max:w.daily.wind_speed_10m_max?.[i]}));
  return {type:'weather',status:'ok',checked_at:new Date().toISOString(),location:{name:loc.name,admin1:loc.admin1,country:loc.country},records:rows,note:'Open-Meteoの7日間予報です。時刻は日本時間。予報は更新されます。'};
}
...
function forcedTool(question){
  ...
  if(/最新|現在|今どう|今日の|明日の|あしたの|今週|週末|来週|発売日|価格|値段|営業時間|営業中|在庫|何時|いつ発売|新しいの|アップデート|バージョン/.test(question))return {ruru_tool:'web_search',query:question};
  return null;
}
async function answerQuestion(channel,member,question,messageId=null){
  ...
  const base={author_username:member.user.username,author_display_name:member.displayName||member.user.globalName||member.user.username,author_id:member.id,channel_id:channel.id,guild_id:channel.guild.id,message_id:messageId||'slash-command',timestamp:new Date().toISOString()};
  const packet={protocol:'ruru-v2',phase:'request',question:clean(question,4000),now:new Date().toISOString(),timezone:'Asia/Tokyo',author:base.author_display_name,recent_messages:history,readable_channels:[...scope.allowed.values()].slice(0,90).map(c=>({id:c.id,name:c.name,topic:clean(c.topic,180)})),can_search_server:true,can_search_news_headlines:true,can_search_web:true,can_search_weather:true};
  ...
  if(tool){
    ...
    answer=await askMake(base,{...packet,phase:'answer',tool_result:result,forced_search:true});
  }else{
    answer=await askMake(base,packet); tool=parseTool(answer);
    ...
    if(tool){answer=await askMake(base,{...packet,phase:'answer',tool_result:result}); if(parseTool(answer))answer='すみません。検索結果をうまく整理できませんでした。もう一度お願いします。';}
  }
  lastTool=tool?.ruru_tool||null;lastReplyAt=new Date().toISOString();return clean(answer,7600);
}
